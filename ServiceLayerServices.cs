...
using _10xErp.ServiceReferenceLayer;
...
namespace _10xErp
{
    public class ServiceLayerServices
    {
        ...
        public DateTime loginTrnsTime { get; set; }
        public bool IsInUse { get; set; }
        ////////////////////////////////////////////////////////////////////////////////////////////////////////
        private ServiceReferenceLayer.SAPB1.ServiceLayer currentServiceContainer = null;     //EntityContainer:  DataServiceContext
        ...
        /// <returns></returns>
        public List<ODataProperty> FilterNullValues(List<ODataProperty> listSource, bool bIgnore = false)
        {
        ...
        public void InitServiceContainer(string strServerURL)
        {
            ...
            if (null == currentServiceContainer)
            {
                ...
                currentServiceContainer = new ServiceReferenceLayer.SAPB1.ServiceLayer(service);
                if (null != currentServiceContainer)
                {
        ...
        public Document GetDeliveryDocumentsDetails(int docEntry)
        {
        ...
        public Document GetSalesOrderDetails(int docEntry)
        {
        ...
        public string GetFOCRemarks(int orderEntry, int linenumber)
        {
            string focRemark = "";
        ...
        public string GetContactPersonName(string cardCode, int? contactPersonCode)
        {
            ...
            // First: fetch the full BusinessPartner object
            var bpResult = currentServiceContainer.BusinessPartners
                .Where(p => p.CardCode == cardCode)
                .ToList(); // Execute the query

            var businessPartner = bpResult.FirstOrDefault();
            if (businessPartner == null || businessPartner.ContactEmployees == null)
                return "";

            // Now filter the ContactEmployees locally (in-memory)
            var contact = businessPartner.ContactEmployees
                .ToList()
                .Find(c => c.InternalCode == contactPersonCode.Value);

            return contact != null ? contact.Name : "";
        }


        public string GetPaymentTermName(int? paymentGroupCode)
        {
            if (!paymentGroupCode.HasValue) return "";

            var term = currentServiceContainer.PaymentTermsTypes
                .Where(p => p.GroupNumber == paymentGroupCode.Value)
                .FirstOrDefault();

            return term != null ? term.PaymentTermsGroupName : "";
        }

        public string GetSalesEmployeeName(int? salesEmpCode)
        {
            if (!salesEmpCode.HasValue) return "";

            var emp = currentServiceContainer.SalesPersons
                .Where(s => s.SalesEmployeeCode == salesEmpCode.Value)
                .FirstOrDefault();

            return emp != null ? emp.SalesEmployeeName : "";
        }

        public string GetSeriesname(int? seriesCode)
        {
            string SeriesName = "";
            string strSeriesvalue = "select \"SeriesName\" from NNM1 where \"Series\"='" + seriesCode +"'";

            DataSet dsData = objHlpr.getDataSet(strSeriesvalue);
            foreach (DataRow dr in dsData.Tables[0].Rows)
            {
                SeriesName = dsData.Tables[0].Rows[0]["SeriesName"].ToString().Trim();
            }
        ...
        public decimal GetTotalDiscSum(int DocEntry)
        {
            ...
            decimal totalLineAmount = GetTotalSum(DocEntry);
        ...
        public decimal GetTotalDiscSumDelivery(int DocEntry)
        {
            Document order = currentServiceContainer.DeliveryNotes
                                  .Where(o => o.DocEntry == DocEntry)
                                  .SingleOrDefault();

            decimal totalLineAmount = GetTotalSumDelivery(DocEntry);
        ...
        /// <param name="order"></param>
        /// <returns></returns>
        public BusinessPartner AddNewCustomer(BusinessPartner customer)
        {
            BusinessPartner newOrderDoc = null;

            try
            {
                currentServiceContainer.AddToBusinessPartners(customer);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
                    ...
                    if (null != retDoc)
                    {
                        newOrderDoc = (BusinessPartner)retDoc;
                    }
            ...
            {
                //Discard the order from beting tracking
                currentServiceContainer.Detach(customer);
                throw ex;
            }
        ...
        /// <summary>
        /// Send Delete Order command
        /// This operation is not supported for Orders and therefore a non supported error will be sent back by Service Layer
        /// </summary>
        /// <param name="docEntry"></param>
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddPurchRequest(Document doc)
        {
            ...
            try
            {
                ...
                currentServiceContainer.AddToPurchaseRequests(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddGoodsReceiptReturn(Document doc)
        {
            Document newDoc = null;
            ...
            try
            {
                ...
                //   currentServiceContainer.AddToIncomingPayments()

                // currentServiceContainer.AddToPurchaseReturns(doc);
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddGoodsReceiptPO(Document doc)
        {
            Document newDoc = null;
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddPO(Document doc)
        {
            Document newDoc = null;
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Item AddItem(Item doc)
        {
            Item newDoc = null;

            try
            {
                currentServiceContainer.AddToItems(doc);
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        //public U_CP_OITM AddItem(U_CP_OITM doc)
        //{
        //    U_CP_OITM newDoc = null;

        //    try
        //    {
        //        currentServiceContainer.AddToU_CP_OITM(doc);
        //        //currentServiceContainer. AddToPurchaseDeliveryNotes(doc);
        //        DataServiceResponse response = currentServiceContainer.SaveChanges();
        //        if (null != response)
        //        {
        ...
        /// <returns></returns>
        /// 
        public Document AddSOdelivery(Document doc)
        {
            Document newDoc = null;

            try
            {
                currentServiceContainer.AddToDeliveryNotes(doc);
                //currentServiceContainer. AddToPurchaseDeliveryNotes(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddGoodsReceiptWOPO(Document doc)
        {
            Document newDoc = null;
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddSalesInvoice(Document doc)
        {
            Document newDoc = null;
            ...
            {
                //currentServiceContainer.AddToDrafts(doc);
                currentServiceContainer.AddToInvoices(doc);
                //currentServiceContainer.AddToPurchaseDeliveryNotes(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Payment AddInvoicePmnt(Payment doc)
        {
            Payment newDoc = null;
            ...
            {
                //currentServiceContainer.AddToDrafts(doc);
                currentServiceContainer.AddToIncomingPayments(doc);
                //currentServiceContainer.AddToPurchaseDeliveryNotes(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddSalesOrder(Document doc)
        ...
        public Document AddDeliveryNote(Document doc)
        {
            Document newDoc = null;

            try
            {
                // Add to the delivery‑notes collection instead of Orders
                currentServiceContainer.AddToDeliveryNotes(doc);

                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (response != null)
                {
                    // There should be exactly one ChangeOperationResponse in the response
                    var opRes = (ChangeOperationResponse)response.SingleOrDefault();
                    if (opRes != null)
                    {
                        // Pull back the materialized entity
                        var descriptor = (System.Data.Services.Client.EntityDescriptor)opRes.Descriptor;
                        newDoc = (Document)descriptor.Entity;
                    }
                }
            }
            catch (Exception ex)
            {
                // Roll back tracking if anything goes wrong
                currentServiceContainer.Detach(doc);
                throw;
            }
        ...
        public Document AddSalesQuotation(Document doc)
        {
            Document newDoc = null;
            ...
            {
                //currentServiceContainer.AddToDrafts(doc);
                currentServiceContainer.AddToQuotations(doc);
                //currentServiceContainer.AddToPurchaseDeliveryNotes(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
        ...
        /// <param name="cardCode"></param>
        /// <returns></returns>
        public PickList GetPickList(int pickid)
        {
            PickList sc = currentServiceContainer.PickLists.Where(cursor => cursor.Absoluteentry == pickid).SingleOrDefault();
            return sc;
        }
        ...
        /// Update a PickList
        /// </summary>
        /// <param name="bpServiceCall"></param>
        public bool UpdatePickList(PickList oPickList)
        {
            try
            {
                //Update the document now
                currentServiceContainer.UpdateObject(oPickList);
                SaveChangesOptions updateSemantics = SaveChangesOptions.ReplaceOnUpdate;
                updateSemantics = SaveChangesOptions.PatchOnUpdate;
            ...
            catch (Exception ex)
            {
                ...
                if (null != oPickList)
                    currentServiceContainer.Detach(oPickList);
        ...
        /// </summary>
        /// <param name="bpServiceCall"></param>
        public bool UpdateBPDoc(BusinessPartner bp)
        {
            try
            {
                //Update the document now
                currentServiceContainer.UpdateObject(bp);
                SaveChangesOptions updateSemantics = SaveChangesOptions.ReplaceOnUpdate;
                updateSemantics = SaveChangesOptions.PatchOnUpdate;
        ...
        /// <summary>
        /// Update a ServiceCall
        /// </summary>
        /// <param name="bpServiceCall"></param>
        public bool UpdateStockTransfer(StockTransfer oStockTransfer)
        {
            try
            {
                //Update the document now
                currentServiceContainer.UpdateObject(oStockTransfer);
                SaveChangesOptions updateSemantics = SaveChangesOptions.ReplaceOnUpdate;
                updateSemantics = SaveChangesOptions.PatchOnUpdate;
            ...
            {
                //Discard the bp from beting tracking
                if (null != oStockTransfer)
                    currentServiceContainer.Detach(oStockTransfer);
        ...
        /// <summary>
        /// Get a BusinessPartner document
        /// </summary>
        /// <param name="cardCode"></param>
        /// <returns></returns>
        public BusinessPartner GetBPDoc(string CardCode)
        {
            BusinessPartner bp = currentServiceContainer.BusinessPartners.Where(cursor => cursor.CardCode == CardCode).SingleOrDefault();
            return bp;
        }
        ...
        /// <summary>
        /// Update a ClientIntake
        /// </summary>
        /// <param name="bpServiceCall"></param>


        /// <summary>
        /// Get a stock Transfer document
        /// </summary>
        /// <param name="cardCode"></param>
        /// <returns></returns>
        public StockTransfer GetStockTransferDoc(int docEntry)
        {
            StockTransfer sc = currentServiceContainer.StockTransfers.Where(cursor => cursor.DocEntry == docEntry).SingleOrDefault();
            return sc;
        }
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public StockTransfer AddToStockTransfers(StockTransfer doc)
        {
            StockTransfer newDoc = null;

            try
            {

                currentServiceContainer.AddToStockTransfers(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        ///// <summary>
        ///// Add a Stock Counting document
        ///// </summary>
        ///// <param name="bp"></param>
        ///// <returns></returns>
        //public MXL_STOCKCOUNTING  AddTOMXLStockTaking(MXL_STOCKCOUNTING doc)
        //{
        //    MXL_STOCKCOUNTING newDoc = null;

        //    try
        //    {

        //        currentServiceContainer.AddToPSTSTOCKCOUNT(doc);
        //        DataServiceResponse response = currentServiceContainer.SaveChanges();
        //        if (null != response)
        //        {
        //            ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        //            if (null != retObj)
        //            {
        //                newDoc = (MXL_STOCKCOUNTING)retObj;
        //            }
        ...
        //}
        /// <summary>
        /// Add a Stock Transfer Request
        /// </summary>
        /// <param name="bp"></param>
        /// <returns></returns>
        public InventoryCounting AddStockTaking(InventoryCounting doc)
        {
            InventoryCounting newDoc = null;

            try
            {

                currentServiceContainer.AddToInventoryCountings(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
                    ...
                    if (null != retObj)
                    {
                        newDoc = (InventoryCounting)retObj;
                    }
        ...
        /// <summary>
        /// Add a Stock Transfer document
        /// </summary>
        /// <param name="bp"></param>
        /// <returns></returns>
        //public ME_OSTR AddToAddonStockTransfer(ME_OSTR doc)
        //{
        //    ME_OSTR newDoc = null;
        //    try
        //    {
        //        currentServiceContainer.AddToME_OSTR(doc);
        //        DataServiceResponse response = currentServiceContainer.SaveChanges();
        //        if (null != response)
        //        {
        //            ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public StockTransfer AddMaterialRequest(StockTransfer doc)
        {
            StockTransfer newDoc = null;
            ...
            try
            {
                ...
                currentServiceContainer.AddToInventoryTransferRequests(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        ///// <param name="bp"></param>
        ///// <returns></returns>
        //public ALE_BARCODE AddBarcodePrintRequest(ALE_BARCODE doc)
        //{
        //    ALE_BARCODE newDoc = null;

        //    try
        //    {

        //        currentServiceContainer.AddToUDO_BARCODE(doc);
        //        DataServiceResponse response = currentServiceContainer.SaveChanges();
        //        if (null != response)
        //        {
        //            ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        /// <summary>
        /// Add a Stock Inward document
        /// </summary>
        /// <param name="bp"></param>
        /// <returns></returns>
        //public ME_OMI AddToAddonStockInward(ME_OMI doc)
        //{
        //    ME_OMI newDoc = null;
        //    try
        //    {
        //        currentServiceContainer.AddToME_OMI(doc);
        //        DataServiceResponse response = currentServiceContainer.SaveChanges();
        //        if (null != response)
        //        {
        //            ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddBranchIssue(Document doc)
        {
            Document newDoc = null;
            try
            {
                //   currentServiceContainer.AddToIncomingPayments()
                currentServiceContainer.AddToInventoryGenExits(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public Document AddBranchReceipt(Document doc)
        {
            Document newDoc = null;
            try
            {
                //   currentServiceContainer.AddToIncomingPayments()
                currentServiceContainer.AddToInventoryGenEntries(doc);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
        ...
        /// <param name="cardCode"></param>
        /// <returns></returns>
        public ServiceCall GetServiceCall(int callID)
        {
            ServiceCall sc = currentServiceContainer.ServiceCalls.Where(cursor => cursor.ServiceCallID == callID).SingleOrDefault();
            return sc;
        }
        ...
        /// <param name="bp"></param>
        /// <returns></returns>
        public ServiceCall Add_ServiceCall(ServiceCall oServiceCall)
        {
            ServiceCall newServiceCall = null;

            try
            {
                currentServiceContainer.AddToServiceCalls(oServiceCall);
                DataServiceResponse response = currentServiceContainer.SaveChanges();
                if (null != response)
                {
                    ChangeOperationResponse opRes = (ChangeOperationResponse)response.SingleOrDefault();
                    ...
                    if (null != retObj)
                    {
                        newServiceCall = (ServiceCall)retObj;
                    }
            ...
            {
                //Discard the order from beting tracking
                currentServiceContainer.Detach(oServiceCall);
                throw ex;
            }
        ...
        /// </summary>
        /// <param name="bpServiceCall"></param>
        public bool UpdateServiceCall(ServiceCall oServiceCall)
        {
            try
            {
                //Update the document now
                currentServiceContainer.UpdateObject(oServiceCall);
                SaveChangesOptions updateSemantics = SaveChangesOptions.ReplaceOnUpdate;
                updateSemantics = SaveChangesOptions.PatchOnUpdate;
