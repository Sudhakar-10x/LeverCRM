...
namespace _10xErp.Controllers
{
    ...
    public class PurchaseController : Controller
    {
        ...
        {
            PurchaseViewModel oPurchaseViewModel = new PurchaseViewModel();

            ViewBag.Current = "PrchaseOrder";
            return View(oPurchaseViewModel);
        }
        ...
        public ActionResult Create()
        {
            ...
            PurchaseViewModel oPurchaseOrderViewModel = new PurchaseViewModel();
            oPurchaseOrderViewModel.ItemDetailsListView = new List<ItemDetails>();
            try
            {
                ...
                oPurchaseOrderViewModel.ReqDate = DateTime.Today;
                oPurchaseOrderViewModel.postingdate = DateTime.Today;
                oPurchaseOrderViewModel.LPODate = DateTime.Today;
                oPurchaseOrderViewModel.series = Convert.ToInt32(ViewBag.seriesId);
                oPurchaseOrderViewModel.DocNum = Convert.ToInt32(ViewBag.DefaultDocNumber);
            ...
            ViewBag.Current = "PurchaseOrder";

            return View(oPurchaseOrderViewModel);
        }
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(sqlCon))
                {
                    ...
                    string query = "SELECT T0.\"DocEntry\", T0.\"DocNum\", T0.\"CardCode\", T0.\"CardName\", T0.\"NumAtCard\", " +
                                   ...
                                   "WHERE T0.\"CANCELED\" = 'N' AND T0.\"ObjType\" = '22' " +
        ...
        public ActionResult PurchaseOrderDetails(int id)
        {
            ...
            var order = currentOdataService.GetPurchaseOrderDetails(id);
            ...
            PurchaseViewModel PURCHSEVIEW = new PurchaseViewModel
            {
            ...
            return View(PURCHSEVIEW);
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create(PurchaseViewModel oPODetails, string custCode)
        {
            ...
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(oPODetails.CardCode))
                    {
                        if (!string.IsNullOrEmpty(oPODetails.RefNo))
                        {
                            if (oPODetails.ItemDetailsListView != null)
                            {
                                ...
                                Document oPrchseOrdrReqst = new Document();

                                if (oPODetails.Staff)
                                {
                                    oPrchseOrdrReqst.ReqType = 171;  //171 -Employee, 12 --User(Supplier)
                                    oPrchseOrdrReqst.Requester = oPODetails.EmpID;
                                }
                                else
                                {
                                    oPrchseOrdrReqst.Requester = "manager";
                                    oPrchseOrdrReqst.ReqType = 12;
                                }

                                // EmpID, Manag
                                oPrchseOrdrReqst.DocObjectCode = "17";
                                //oPurReqst.DocType = "dDocument_Items";
                                oPrchseOrdrReqst.SeriesString = "";
                                oPrchseOrdrReqst.Series = oPODetails.series;
                                oPrchseOrdrReqst.DocNum = oPODetails.DocNum;
                                oPrchseOrdrReqst.CardCode = oPODetails.CardCode;
                                //oPrchseOrdrReqst.RequriedDate = Convert.ToDateTime(oPODetails.ReqDate);
                                oPrchseOrdrReqst.DocDueDate = Convert.ToDateTime(oPODetails.ReqDate);
                                oPrchseOrdrReqst.Comments = @Session["UserName"] + " " + oPODetails.Remarks;
                                oPrchseOrdrReqst.DocDate = Convert.ToDateTime(oPODetails.postingdate);
                                //oPrchseOrdrReqst.DocDueDate = DateTime.Today.Date;
                                oPrchseOrdrReqst.TaxDate = Convert.ToDateTime(oPODetails.LPODate);
                                oPrchseOrdrReqst.NumAtCard = oPODetails.RefNo;
                                oPrchseOrdrReqst.U_DelLocation = oPODetails.deliveryLocation;
                                oPrchseOrdrReqst.U_PUser = @Session["UserName"].ToString();
                                oPrchseOrdrReqst.U_IsFrmPrtal = "Y";
                                oPrchseOrdrReqst.ContactPersonCode = !string.IsNullOrEmpty(oPODetails.contactPerson) ? 0 : Convert.ToInt32(oPODetails.contactPerson);
                                //oPrchseOrdrReqst.U_PDTUSER = oPODetails.EmpID;

                                //oPrchseOrdrReqst.U_LocationName = getLocationName(oPODetails.WhseCode);
                                //oPrchseOrdrReqst.U_UserName = getEmpName(oPODetails.EmpID);
                                //DocSeries oDocSeries = getSeriesInfo("Sales Order", oPODetails.WhseCode);
                                ...
                                //oPrchseOrdrReqst.Series = 8;
                                var seObj = GetSalesEmployeeInfoByUId(oPODetails.EmpID);
                                oPrchseOrdrReqst.BPL_IDAssignedToInvoice = Convert.ToInt32(oPODetails.BranchId);
                                oPrchseOrdrReqst.PaymentGroupCode = Convert.ToInt32(oPODetails.Paymentterms);
                                //oPrchseOrdrReqst.NumAtCard = oPODetails.CustRefNo;
                                oPrchseOrdrReqst.DiscountPercent = Convert.ToDouble(oPODetails.DocDiscount);
                                oPrchseOrdrReqst.SalesPersonCode = seObj.AssignedEmpId;
                                oPrchseOrdrReqst.ShipToCode = oPODetails.Shiptocode;
                                oPrchseOrdrReqst.PayToCode = oPODetails.Billtocode;
                                oPrchseOrdrReqst.SalesPersonCode = Convert.ToInt32(oPODetails.Salesemp);
                                DocumentLine oLine = null;
                                int iLine = 0;
                                foreach (ItemDetails itm in oPODetails.ItemDetailsListView)
                                {
                                    oLine = new DocumentLine();

                                    oLine.ItemCode = itm.ItemCode;
                                    oLine.LineVendor = oPODetails.CardCode;
                                    ...
                                    oLine.DiscountPercent = Convert.ToDouble(itm.DisPer);
                                    oPrchseOrdrReqst.DocumentLines.Add(oLine);
                                ...
                                doc = currentOdataService.AddPurchaseOrder(oPrchseOrdrReqst);
                                //var msg = "<div style='width:800px;margin:auto;padding:40px;border:1px solid #abd1b8;'><h2>10XSF</h2><h3>Hello, " + oPODetails.CardName + "</h3><h4>Your sales order has been successfully created!</h4><div style='background:#abd1b8;padding:20px 10px;'><p>Sales Order No: <b style='color:green'>" + doc.DocNum + "</b></p></div></div>";
                                //var email = this.getCustomerEmail(oPODetails.CardCode);
                                //SendCodeToEmail("raju.m432@gmail.com", msg);
                                //UpdateRequestTime();
                                cnfMsg.IsSucess = true;
                                cnfMsg.CnfsMsg = "Purchase Order  Document Number :" + doc.DocNum + " Created Sucessfully";
                                TempData["SuccessMessage"] = "Purchase Order  Document Number :" + doc.DocNum + " Created Sucessfully";
                            }
                            else
                                TempData["ErrorMessage"] = "Make sure correct data to generate Purchase order !!";
                        }
                        else
                            TempData["ErrorMessage"] = "Please write Vendor Ref to generate Purchase order !!";
                    }
                }
                else
                {
                    oPODetails.CardCode = oPODetails.CardCode ?? Request.Form["CustomerID"];
                    oPODetails.CardName = oPODetails.CardName ?? Request.Form["customerAutocomplete"];

                    custCode = oPODetails.CardCode ?? Request.Form["CustomerID"];

                    LoadDropdowns(custCode); // Load dropdowns again so page doesn't break

                    if (oPODetails.ItemDetailsListView == null)
                        oPODetails.ItemDetailsListView = new List<ItemDetails>();
                    Utilities.SetResultMessage("Make sure correct data to generate purchase order !!");
                    TempData["ErrorMessage"] = "Make sure correct data to generate purchase order !!";
                }
            }
            catch (Exception ex)
            {
                ...
                Utilities.SetResultMessage("PurchaseOrder" + ex.Message.ToString());
                ...
                if (ex.InnerException != null)
                {
                    ...
                    {
                        cnfMsg.IsSucess = true;
                        Excpn = "Purchase order went for Approval Successfully";
                        //cnfMsg.CnfsMsg = "Sales order went for Approval Successfully";
                        cnfMsg.Ref = "APPROVAL";
                        TempData["SuccessMessage"] = "Purchase order went for Approval Successfully";
        ...
        public JsonResult GetCustomer(string CustId)
        {
            ...
            try
            {
                ...
                string query = "SELECT \"CardCode\", \"CardName\",\"SlpCode\",\"ListNum\"," +
                    ...
                    " (select \"ExtraDays\" from OCTG where \"GroupNum\"=OCRD.\"GroupNum\") as \"ExtraDays\" From OCRD WHERE \"CardType\" = 'S' AND \"frozenFor\" = 'N' "
        ...
        public List<ItemDetails> GetAllItems()
        {
            ...
            try
            {
                string strSQL = "select \"ItemCode\",\"ItemName\" from OITM where \"validFor\"='Y' and \"PrchseItem\"='Y'";
                DataSet dsData = objHlpr.getDataSet(strSQL);
                foreach (DataRow dr in dsData.Tables[0].Rows)
                {
        ...
        private List<SelectListItem> GetSeriesList()
        {
            ...
            string query = "SELECT Series,SeriesName, ISNULL(NextNumber,0) as NextNumber FROM NNM1 WHERE ObjectCode = '22' and Indicator like '%" + DateTime.Now.Year.ToString() + "%' ";
        ...
        public JsonResult GetItemList(string searchTerm, string customertype)
        {
            ...
            using (SqlConnection conn = new SqlConnection(sqlCon))
            {
               ...
               FROM OITM  
               WHERE (ItemName LIKE @SearchTerm OR ItemCode LIKE @SearchTerm) and validFor='Y' and PrchseItem ='Y' ";
