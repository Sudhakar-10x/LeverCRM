...
namespace _10xErp.Controllers
{
    public class DeliveryController : Controller
    {
        ...
        public static List<DeliveryOrderViewModel> DOrders = new List<DeliveryOrderViewModel>(); // In-memory store  for Delivery Screen


        public ActionResult Index()
        {
            DeliveryOrderViewModel oDlrvyGenReqModel = new DeliveryOrderViewModel();

            ViewBag.Current = "Delivery";
            return View(oDlrvyGenReqModel);
        }
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(sqlCon))
                {
                    ...
                    string query = "SELECT T0.\"DocEntry\", T0.\"DocNum\", T0.\"CardCode\", T0.\"CardName\", T0.\"NumAtCard\", " +
                                   ...
                                   "T2.\"USER_CODE\", SUM(T3.\"Quantity\") AS \"TotalQty\" " +
                                   "FROM \"ODLN\" T0 " +
                                   ...
                                   "WHERE T0.\"CANCELED\" = 'N' AND T0.\"DocStatus\" = 'O' AND T2.\"USER_CODE\" = '" + username + "' " +
                                   "AND T0.\"CreateDate\" BETWEEN '" + fromDate + "' AND '" + toDate + "' " +
                                   "GROUP BY T0.\"DocEntry\", T0.\"DocNum\", T0.\"CardCode\", T0.\"CardName\", T0.\"NumAtCard\", " +
                                   "T0.\"CreateDate\", T0.\"DocTotal\", T0.\"UserSign\", T1.\"U_NAME\", T2.\"USER_CODE\" " +
        ...
        public ActionResult DeliveryDocumentDetails(int id)
        {
            ...
            var order = currentOdataService.GetDeliveryDocumentsDetails(id);
            ...
            DeliveryOrderViewModel DELIVERYVIEW = new DeliveryOrderViewModel
            {
                ...
                TotalBeforeDiscount = currentOdataService.GetTotalSumDelivery(Convert.ToInt32(order.DocEntry)),
                DocDiscount = currentOdataService.GetTotalDiscSumDelivery(Convert.ToInt32(order.DocEntry)),
        ...
        [HttpGet]
        public ActionResult Create(string custCode)
        {
            ViewBag.UserName = User.Identity.Name;
            DeliveryOrderViewModel oDelvryOrderViewModel = new DeliveryOrderViewModel();
            oDelvryOrderViewModel.ItemDetailsListView = new List<ItemDetails>();
            try
            {

                oDelvryOrderViewModel.ItemDetailsListView = new List<ItemDetails>();
                //oSalesOrderViewModel.SalesEmployeeList = new SelectList(GetSalesEmployeeList(), "Value", "Text");
                LoadDropdowns(custCode);

                oDelvryOrderViewModel.ReqDate = DateTime.Today;
                oDelvryOrderViewModel.postingdate = DateTime.Today;
                oDelvryOrderViewModel.LPODate = DateTime.Today;
                oDelvryOrderViewModel.series = Convert.ToInt32(ViewBag.seriesId);
                oDelvryOrderViewModel.DocNum = Convert.ToInt32(ViewBag.DefaultDocNumber);
            ...
            return View(oDelvryOrderViewModel);
        }


        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create(DeliveryOrderViewModel oDlrvyDetails, string custCode)
        {
            ...
            try
            {
                ...
                {
                    if (oDlrvyDetails.ItemDetailsListView != null)
                    {
                        ...
                        if (oDlrvyDetails.Staff)
                        {
                            oDelvryReqst.ReqType = 171;  //171 -Employee, 12 --User(Supplier)
                            oDelvryReqst.Requester = oDlrvyDetails.EmpID;
                        }
                        else
                        {
                        ...
                        oDelvryReqst.DocObjectCode = "15";

                        oDelvryReqst.SeriesString = "";
                        oDelvryReqst.Series = oDlrvyDetails.series;
                        oDelvryReqst.DocNum = oDlrvyDetails.DocNum;
                        oDelvryReqst.CardCode = oDlrvyDetails.CardCode;
                        oDelvryReqst.RequriedDate = Convert.ToDateTime(oDlrvyDetails.ReqDate);
                        oDelvryReqst.DocDueDate = Convert.ToDateTime(oDlrvyDetails.ReqDate);
                        oDelvryReqst.Comments = @Session["UserName"] + " " + oDlrvyDetails.Remarks;
                        oDelvryReqst.DocDate = Convert.ToDateTime(oDlrvyDetails.postingdate);

                        oDelvryReqst.NumAtCard = oDlrvyDetails.RefNo;
                        oDelvryReqst.U_DelLocation = oDlrvyDetails.deliveryLocation;

                        var seObj = GetSalesEmployeeInfoByUId(oDlrvyDetails.EmpID);
                        oDelvryReqst.BPL_IDAssignedToInvoice = Convert.ToInt32(oDlrvyDetails.BranchId);
                        oDelvryReqst.PaymentGroupCode = Convert.ToInt32(oDlrvyDetails.Paymentterms);

                        oDelvryReqst.DiscountPercent = Convert.ToDouble(oDlrvyDetails.DocDiscount);
                        oDelvryReqst.SalesPersonCode = seObj.AssignedEmpId;
                        oDelvryReqst.ShipToCode = oDlrvyDetails.Shiptocode;
                        oDelvryReqst.PayToCode = oDlrvyDetails.Billtocode;
                        oDelvryReqst.SalesPersonCode = Convert.ToInt32(oDlrvyDetails.Salesemp);
                        DocumentLine oLine = null;
                        int iLine = 0;
                        foreach (ItemDetails itm in oDlrvyDetails.ItemDetailsListView)
                        {
                            oLine = new DocumentLine();

                            oLine.ItemCode = itm.ItemCode;
                            oLine.LineVendor = oDlrvyDetails.CardCode;
                        ...
                        doc = currentOdataService.AddDeliveryNote(oDelvryReqst);

                        var msg = "<div style='width:800px;margin:auto;padding:40px;border:1px solid #abd1b8;'><h2>10XSF</h2><h3>Hello, " + oDlrvyDetails.CardName + "</h3><h4>Your Delivery has been successfully created!</h4><div style='background:#abd1b8;padding:20px 10px;'><p>Delivery No: <b style='color:green'>" + doc.DocNum + "</b></p></div></div>";
                ...
                {
                    // Ensure CardName and CardCode are preserved from the form
                    oDlrvyDetails.CardCode = oDlrvyDetails.CardCode ?? Request.Form["CustomerID"];
                    oDlrvyDetails.CardName = oDlrvyDetails.CardName ?? Request.Form["customerAutocomplete"];

                    custCode = oDlrvyDetails.CardCode ?? Request.Form["CustomerID"];

                    LoadDropdowns(custCode); // Load dropdowns again so page doesn't break

                    if (oDlrvyDetails.ItemDetailsListView == null)
                        oDlrvyDetails.ItemDetailsListView = new List<ItemDetails>();

                    return View(oDlrvyDetails);
                }
            }

            catch (Exception ex)
            {
                Utilities.SetResultMessage("Delivery Error: " + ex.Message);

                string Excpn = ex.Message;
                if (ex.InnerException != null)
                {
                    Excpn += " | Inner Exception: " + ex.InnerException.Message;
                }

                // Set ViewBag error message
                cnfMsg.CnfsMsg = "Error while creating delivery: " + Excpn;
                ViewBag.ErrorMessage = cnfMsg.CnfsMsg;

                // Preserve form values in case of failure
                oDlrvyDetails.CardCode = oDlrvyDetails.CardCode ?? Request.Form["CustomerID"];
                oDlrvyDetails.CardName = oDlrvyDetails.CardName ?? Request.Form["customerAutocomplete"];
        ...
        public JsonResult GetCustomerListForDropDown(string customertype, string searchTerm)
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(sqlCon))
                {
                    ...
                    SELECT CardCode as CustomerID, CardName as Name, '' as PhoneNo, E_Mail as EmailID
                    FROM OCRD
                    WHERE CardType = @type and (CardName LIKE @SearchTerm OR CardCode LIKE @SearchTerm)"; // Filter by type if needed
        ...
        private List<SelectListItem> GetSeriesList()
        {
            ...
            string query = "SELECT Series,SeriesName, ISNULL(NextNumber,0) as NextNumber FROM NNM1 WHERE ObjectCode = '15' and Indicator like '%" + DateTime.Now.Year.ToString() + "%' ";
        ...
        public ActionResult GetItemDetails(string itemCode)
        {
            ...
            try
            {
                ...
                var objitemquery = "SELECT T0.\"ItemCode\",T0.\"ItemName\",T3.\"ItmsGrpNam\",T0.\"OnHand\",T0.\"IsCommited\"," +
                    " T0.\"FrgnName\",T2.\"UomEntry\",T2.\"UomCode\",T0.\"ManBtchNum\",T0.\"ManSerNum\",T0.\"VatGourpSa\", " +
                ...
                if (dsData.Tables[0].Rows.Count > 0)
                {
                    ...
                    // Safe parsing for Discount
                    decimal.TryParse(dsData.Tables[0].Rows[0]["Discount"]?.ToString(), out disPer);

                    // Safe parsing for Price
                    decimal.TryParse(dsData.Tables[0].Rows[0]["Price"]?.ToString(), out price);

                    objItemDetails = new ItemDetails()
                    {
                        ItemCode = dsData.Tables[0].Rows[0]["ItemCode"]?.ToString() ?? string.Empty,
                        ItemName = dsData.Tables[0].Rows[0]["ItemName"]?.ToString() ?? string.Empty,
                        DisPer = disPer,
                        Price = price,
                        UomName = dsData.Tables[0].Rows[0]["UomCode"]?.ToString() ?? string.Empty,
                        Warehouse = dsData.Tables[0].Rows[0]["Warehouse"]?.ToString() ?? string.Empty,
                        VatGrpCode = dsData.Tables[0].Rows[0]["VatGourpSa"]?.ToString() ?? string.Empty,
                        foc = "No"
                    };
                }
